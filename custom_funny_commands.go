...
func getRandomEntry(ra *randomiseArray) string {
	return ra.lines[rand.Intn(ra.count)]
}

func initialiseRandomArray(input string, failure string) *randomiseArray {
	rand.Seed(time.Now().UnixNano()) //should be run at least once
	randomarray := &randomiseArray{}
	randomarray.lines = strings.Split(strings.ReplaceAll(input, "\r\n", "\n"), "\n")
	randomarray.count = len(randomarray.lines)-1
	if randomarray.count <= 0 {
		randomarray.lines = []string{failure}
		randomarray.count = 1
	}
	return randomarray
}
...
func HandleRandomDadjokeHandler() CommandCallback {
	dadjokearray := initialiseRandomArray(dadJokesFile, "No Dad Jokes found")

	return func(ca CommandArgs) BotReply {
		return BotPrintSimple(ca.room, getRandomEntry(dadjokearray))
	}
}
...
func HandleRandomQuoteHandler() CommandCallback {
	quotesarray := initialiseRandomArray(quotesFile, "No Quotes found")
	
	return func(ca CommandArgs) BotReply {
		return BotPrintSimple(ca.room, getRandomEntry(quotesarray))
	}
}
...
func HandleGod(ca CommandArgs) BotReply {
	return BotPrintSimple(ca.room, getGodsWords(ca.argc))
}


func HandleHello(ca CommandArgs) BotReply {
	return BotPrintPinged(ca.room, &ca.sender, "Hello, ", "!")
}
