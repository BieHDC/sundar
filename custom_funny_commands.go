...
//go:embed misc_files/dad_jokes.txt
var dadJokesFile string

//go:embed misc_files/quotes.txt
var quotesFile string

type randomiseArray struct {
	lines []string
	count int //for one we need to cap -1 due to last line and we dont need to len() it in every call
}

var dadjokearray *randomiseArray
func initialiseDadjokes() {
	dadjokearray = &randomiseArray{}
	dadjokearray.lines = strings.Split(strings.ReplaceAll(dadJokesFile, "\r\n", "\n"), "\n")
	dadjokearray.count = len(dadjokearray.lines)-1
	if dadjokearray.count <= 0 {
		dadjokearray.lines = []string{"No Dad Jokes found"}
		dadjokearray.count = 1
	}
}

func getRandomDadjoke(cmdhdlr *CommandHandler) string {
	return dadjokearray.lines[rand.Intn(dadjokearray.count)]
}

func HandleRandomDadjoke(cmdhdlr *CommandHandler, room id.RoomID, sender id.UserID, argc int, argv []string, statusroom id.RoomID, evt *event.Event) bool {
	BotReplyMsg(cmdhdlr, room, getRandomDadjoke(cmdhdlr))
	return true
}

var quotesarray *randomiseArray
func initialiseQuotes() {
	quotesarray = &randomiseArray{}
	quotesarray.lines = strings.Split(strings.ReplaceAll(quotesFile, "\r\n", "\n"), "\n")
	quotesarray.count = len(quotesarray.lines)-1
	if quotesarray.count <= 0 {
		quotesarray.lines = []string{"No Quotes found"}
		quotesarray.count = 1
	}
}

func getRandomQuote(cmdhdlr *CommandHandler) string {
	return quotesarray.lines[rand.Intn(quotesarray.count)]
}

func HandleRandomQuote(cmdhdlr *CommandHandler, room id.RoomID, sender id.UserID, argc int, argv []string, statusroom id.RoomID, evt *event.Event) bool {
	BotReplyMsg(cmdhdlr, room, getRandomQuote(cmdhdlr))
	return true
}
...
func getGodsWords(count int) string {
	if count < 1 {
		count = 1
	}
	if count > 100 {
	...
	godsong := "God says: "
	for i := 0; i < count; i++ {
		godsong += corpus[rand.Intn(corpuslen)] + " "
	}

	return godsong
}

func HandleGod(cmdhdlr *CommandHandler, room id.RoomID, sender id.UserID, argc int, argv []string, statusroom id.RoomID, evt *event.Event) bool {
	BotReplyMsg(cmdhdlr, room, getGodsWords(argc))
	return true
}

func HandleHello(cmdhdlr *CommandHandler, room id.RoomID, sender id.UserID, argc int, argv []string, statusroom id.RoomID, evt *event.Event) bool {
	BotReplyMsgPinged(cmdhdlr, sender, room, "Hello, ", "!")
	return true
}
